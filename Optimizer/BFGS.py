...
class BFGS:
    ...
    def check_gradient(self, x_test=None, h=1e-5, tol=1e-5):
        ...
        # 计算解析梯度
        f_analytic = self.energy(self.temp_x, self.temp_grad)
        ...
        for i in range(self.dim):
            ...
            f_plus = self.energy(self.temp_x, self.temp_grad)

            # 负向扰动
            x_minus = x_test_np.copy()
            x_minus[i] -= h
            self.temp_x.from_numpy(x_minus)
            f_minus = self.energy(self.temp_x, self.temp_grad)
    ...
    def minimize(self,max_iter=200):
        ...
        for _ in range(max_iter):
            ...
            def calc_grad_norm() -> ti.f32:
                grad_n = 0.0
                for i in range(self.dim): 
                    grad_n += self.grad[i] ** 2
                return grad_n
            grad_norm=calc_grad_norm()
